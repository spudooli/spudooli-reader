...
def cleanupfeeditems(feedid, feedItemCount):
    ...
    cursor = connection.cursor(buffered = True)
    deletefrom = "DELETE FROM feed_items WHERE feed_id = %s and id IN (select id from (SELECT id, feed_id FROM feed_items where feed_id = %s and star is NULL ORDER BY id ASC LIMIT %s) foo)"
